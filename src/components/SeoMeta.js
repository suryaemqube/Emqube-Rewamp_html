...
const Seo = ({ bodyClass, title, description, pageUrl = "emquberevamphtml.netlify.app", seoData, children, visibility = true, }) => {
  ...
  return (
    <>
      ...
      <meta name="twitter:site" content={seoData && seoData.opengraphSiteName ? seoData.opengraphSiteName : "Emqube"} />

      

      <script type="application/ld+json">{Object.keys(jsonObject).length !== 0 ? JSON.stringify(jsonObject, null, 2) : "{}"}</script>
      {/* <meta name="google-site-verification" content="jkJxFLv5s_ewFhbArQvp-cUEKqkVJFAl8WVLs-sxB7A" /> */}
      {children}
    </>
  );
};
