...
export default function SoftwareSolChild({ data }) {
  ...
  const IndusLeading = data?.wpPage?.industryLeadingIntelligencePlatform || {};
    ...
    <>
      ...
      {IndusLeading && 
        <section className="business-inte-wrapper">
          <div className="container">
            <h2 dangerouslySetInnerHTML={{__html: IndusLeading.iliTitle}} />
            <ul className="business-inte-list">
              {IndusLeading?.iliList.map((indusLst, index) => (
                <li>
                  <p className="tech-name" dangerouslySetInnerHTML={{__html: indusLst.iliTechTitle}} />
                  <div className="left">
...
export const Head = ({ data }) => {
  const schema = {
    ...
    "@type": "Service",
    "name": "Business Intelligence & Data Analytics Services Dubai",
    "description": "Transform complex data into actionable insights using intuitive dashboards with Power BI, Tableau, and Looker.",
    ...
    "serviceOutput": "BI Dashboards, Data Warehouses & Analytics Reports"
  };
  return (
    <>
...
export const data = graphql`
  query MyQuery {
    wpPage(databaseId: {eq: 112}) {
      ...
      industryLeadingIntelligencePlatform {
        iliTitle
        iliList {
          iliTechText
          iliTechTitle
