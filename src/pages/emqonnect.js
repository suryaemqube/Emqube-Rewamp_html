...
export default function EmqonnectList({ data }) {

  const emqoList =
    data?.allWpBlog?.edges?.map(edge => edge.node) || [];

  const emqoPage = data?.wpPage || {};
  ...
  // extract unique blog categories from posts
    const categories = [
      ...new Map(
        emqoList
          .flatMap(post => post.blogCategories?.nodes || [])
    ...
    ? emqoList
    : emqoList.filter(post =>
        post.blogCategories?.nodes?.some(cat => cat.slug === selectedCategory)
      );
  ...
  const getPageNumbers = (currentPage, totalPages) => {
    ...
    if (totalPages <= 7) {
      // show all if total pages are few
  ...
  // script for pagination - ends
    ...
    // trim content for blog - starts
    const trimContent = (html, wordLimit = 100) => {
      if (!html) return "";
      ...
      {emqoList && 
        <section className="emqonnect-list-wrapper" >
          <div className="container">
            ...
            <div className={`emqonnect-list ${isLoading ? "is-loading" : ""}`} ref={listingSectionRef}>
